...
class MNISTEncoder(gluon.HybridBlock):
    ...
    def __init__(self, nfilter=32, latent_dim=10, act="elu"):
        ...
        # [3 x 3] => 100
        self.fc = nn.Dense(100, in_units=nfilter * 3 * 3)
        # To mu and logvar
        self.enc_mu = nn.Dense(latent_dim, in_units=100)
        self.enc_logvar = nn.Dense(latent_dim, in_units=100)
...
class MNISTDecoderOnePar(gluon.HybridBlock):
    ...
    def __init__(self, nfilter=32, latent_dim=10, act="elu"):
        ...
        # latent_dim => 100
        self.latent_to_fc = nn.Dense(100, in_units=latent_dim)
        # 100 => [3 x 3]
        self.fc_to_dec = nn.Dense(nfilter * 3 * 3, in_units=100)
        # [3 x 3] => [7 x 7]
...
model = LatentModel(ConditionalBernoulli(),
                    ...
                    sim_z=10, nchain=50, ctx=ctx)
model.init(lr=0.0001, lr_bc=0.001)
...
particles = model.fit_bc(x, epochs=epochs_bc, warmups=warmups_bc, batch_size=batch_size,
                         burnin=burnin, step_size=ss,
                         eval_nll=False, verbose=True)
model.module.save_parameters(filename)
np.save("../results/particles.npy", particles.asnumpy())
# model.module.load_parameters(filename)
...
particles = np.load("../results/particles.npy")
coords = np.mean(particles, axis=0)
samp_ind = range(10000)
coords = coords[samp_ind, :]
labels = y.asnumpy()[samp_ind]

# Save coordinates and use R to plot
pdat = np.append(coords, labels.reshape(-1, 1), axis=1)
np.savetxt("../results/mnist-2d.txt", pdat)
